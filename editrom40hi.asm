; PET/CBM Editor ROM Project - CODEBASE 0 - Brings in Extended Features
; ==========================   EDITROM40HI.ASM
;
...
; If OPTROM is greater than 0 then this code will be put in the OPTION ROM space.
		...
		!IF OPTROM=0 { *=$E900 }  				; Assemble to option ROM at $E900
...
;---------------------- These features can stand alone
	...
	!IF (SS40 + COLOURPET) >0 {
		!SOURCE "editrom40cls.asm"				; Relocate Screen clear
		!SOURCE "editrom40lm.asm"				; Relocate CURSOR_LM code
		!SOURCE "editrom40scrollup.asm"				; Relocate scroll up code
	}
...
;---------------------- Full ColourPET support
	...
	!IF COLOURPET>0 {
		...
		!SOURCE "colourpet40.asm"				; colour subs for CODEBASE 1 and 2
	}
